...
import {
  ...
  Zap, Truck, Lock, Share, Layers, FileDown, Eye, EyeOff, CheckCircle, Shield
} from 'lucide-react';
      ...
      <section className="flex flex-col lg:flex-row justify-between items-end gap-10 page-transition">
        <div className="flex-1 space-y-4">
          <div className="flex items-center gap-3 text-white/40 text-sm font-bold uppercase tracking-[0.4em]">
            <Activity size={14} className="text-blue-400" /> Diagnostic Link Active · v4.2
          </div>
        ...
        <div className="xl:col-span-2 apple-card overflow-hidden !bg-white/[0.01] relative flex flex-col items-center justify-center min-h-[600px]">
          ...
          <div className="absolute bottom-10 flex gap-4">
            <button onClick={handleDownloadPDF} disabled={!result} className="apple-button flex items-center gap-2 disabled:opacity-30 disabled:cursor-not-allowed">
              {pdfUnlocked ? <><CheckCircle size={16} /><span>PDF Ready</span></> : <><Lock size={16} /><span>Unlock Full Report</span></>}
            </button>
          ...
          >
            <div className="flex items-center justify-between border-b border-white/10 pb-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-3xl font-extrabold tracking-tight">Diagnostic Analysis</h3>
                  <p className="text-white/30 text-xs font-bold uppercase tracking-widest mt-1">AI Strategic Intelligence Feed</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Executive Overview</h4>
                <p className="text-white/70 text-sm leading-relaxed font-medium">{ai.overview || 'Analysis pending...'}</p>
              </div>
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Brand Positioning</h4>
                <p className="text-white/70 text-sm leading-relaxed font-medium">{ai.positioning || 'Analyzing market placement...'}</p>
              </div>
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Target Audience</h4>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 border-t border-white/10 pt-10">
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Trust & Conversion Gaps</h4>
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Priority Corrections</h4>
                <ul className="space-y-2">
              ...
              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/30">Quick Wins (7 Days)</h4>
                <ul className="space-y-2">
        ...
        {result && (
          ...
          >
            <div className="flex justify-between items-end">
              <div>
                <h3 className="text-4xl font-extrabold tracking-tighter">Diagnostic Telemetry</h3>
                <p className="text-white/30 uppercase tracking-widest text-[10px] font-bold mt-1">System Node Status · Logic Layer v4.2</p>
              </div>
              <div className="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-2xl border border-white/10">
                <div className="w-2 h-2 rounded-full bg-[#30D158] animate-pulse shadow-[0_0_8px_#30D158]" />
                <span className="text-xs font-black uppercase tracking-widest opacity-60">Feed Active</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
              ...
              <div className="space-y-8">
                <h5 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Integrity Nodes</h5>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={i} className="bg-white/[0.03] border border-white/5 p-5 rounded-2xl">
                      <div className="text-white/30 text-[9px] font-black uppercase mb-1">{n.l}</div>
                      <div className="text-2xl font-black text-white">{n.v.toFixed(1)}</div>
              ...
              {/* Shard Feed blurred if not paid */}
              <div className="relative space-y-6">
                <h5 className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">Diagnostic Shards</h5>
                    ...
                    <div key={f.code} className="flex justify-between items-center py-3 border-b border-white/[0.03] group transition-colors">
                      <div className="flex flex-col">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[8px] text-white/20 font-black uppercase tracking-widest">{f.category}</span>
                          <div className="w-1 h-1 rounded-full bg-white/10" />
                          <span className={`text-[8px] font-black uppercase tracking-widest ${f.severity === 'CRITICAL' ? 'text-red-400' : 'text-blue-400'}`}>
                            {f.severity}
                          </span>
                        </div>
                      </div>
                      <div className="flex flex-col items-end">
                        ...
                        <span className="text-white/20 text-[8px] font-bold uppercase tracking-widest">Effect Vector</span>
                      </div>
                    </div>
                  )) : (
                ...
                {!pdfUnlocked && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center gap-5 z-20">
                    ...
                    <button onClick={() => setShowPaywall(true)} className="apple-button-primary flex items-center gap-2 text-xs px-6 py-3">
